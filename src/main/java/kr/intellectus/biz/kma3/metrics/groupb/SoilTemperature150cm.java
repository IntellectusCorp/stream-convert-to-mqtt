...
public class SoilTemperature150cm extends Measurement {

    static final kr.intellectus.util.Logger logger = kr.intellectus.util.Loggers.getLogger(SoilTemperature150cm.class);

    private final static Metrics metric = Metrics.SOIL_TEMPERATURE_150CM;
    private final static int BITS_OFFSET = 11;
    public final static int OFFSET_BYTES = 2;

    public SoilTemperature150cm(final byte[] bytes) {
        super(metric, bytes, BITS_OFFSET);
    }

    public SoilTemperature150cm(final int represented) {
