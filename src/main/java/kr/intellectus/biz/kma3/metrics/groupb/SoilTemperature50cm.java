...
public class SoilTemperature50cm extends Measurement {

    static final kr.intellectus.util.Logger logger = kr.intellectus.util.Loggers.getLogger(SoilTemperature50cm.class);

    private final static Metrics metric = Metrics.SOIL_TEMPERATURE_50CM;
    private final static int BITS_OFFSET = 11;
    public final static int OFFSET_BYTES = 2;

    public SoilTemperature50cm(final byte[] bytes) {
        super(metric, bytes, BITS_OFFSET);
    }

    public SoilTemperature50cm(final int represented) {
